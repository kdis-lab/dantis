...
class MultinomialNBClassOD(algorithmbase.AlgorithmBase):
    ...
    def __init__(self, hyperparameter: dict, x_train: np.array, y_train: np.array, x_test: np.array = None,
        ...
        if np.sum(x_train < 0) > 0 or np.sum(x_test < 0):
            raise utils.SupervisedInputDataError("Multinomial NB does not accept negative values in data input, we recommend you to scale your data")
    ...
    def fit(self):
        """
            Fit detector. Adjusts the Multinomial_NB model using the training data.
        ...
        proba_anormalidad = super().get_probabilidad_anom(x_train)
    ...
    def predict(self):
        """
            ...
            Adjusts the Multinomial_NB model using the test data.

            Returns:
                Predict probability of outlier
        """
        x_test = super().get_x_test()
        proba_anormalidad = super().get_probabilidad_anom(x_test)
    ...
    def save_model(self, path_model):
        """
            Saves the Multinomial_NB model to the specified file.
    ...
    def load_model(self, path_model):
        """
            Loads the Multinomial_NB model from the specified file.
    ...
    def _create_model(self):
        """
            Creates the Multinomial_NB model with the provided hyperparameters.
        """
        self.set_hyperparameter(self._hyperparameter)
        self._model = MultinomialNB(**self._hyperparameter)