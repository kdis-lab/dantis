from pyod.models import auto_encoder # auto_encoder.AutoEncoder
import numpy as np
import logging
...
class AutoEncoder(algorithmbase.AlgorithmBase):
    """
    AutoEncoder(hyperparameter: dict)
    AutoEncoder for anomaly detection.
    This class implements an AutoEncoder architecture for unsupervised anomaly detection, learning to reconstruct normal data and identifying anomalies based on reconstruction error. It inherits from `algorithmbase.AlgorithmBase` and provides methods for model training, prediction, and anomaly scoring.
    Parameters
    ----------
    hyperparameter : dict
    ...
    ----------
    _model : object
        The underlying AutoEncoder model instance.
    _hyperparameter : dict
        Dictionary of hyperparameters for the model.
    Methods
    -------
    decision_function(x)
        Computes the anomaly score for the input data `x` using the trained AutoEncoder model.
    fit(x_train: np.ndarray, y_train: np.ndarray = None)
        Trains the AutoEncoder model on the provided training data.
    predict(x: np.ndarray = None)
    ...
    - The anomaly score is typically based on the reconstruction error between the input and its reconstruction.
    - Input data should be preprocessed according to the requirements of the underlying AutoEncoder implementation.
    """
    def __init__(self, hyperparameter: dict):
        """
        Initialize the AutoEncoder model with the given hyperparameters.
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray = None) -> np.ndarray: 
        ...
        """
        logging.info("Fitting AutoEncoder model...")
        logging.info("Don't use y_train in AutoEncoder model fitting, it is not used.")
    ...
    def set_hyperparameter(self, hyperparameter):
        ...
        self._model = auto_encoder.AutoEncoder
    ...
    def _create_model(self):
        ...
        self._model = auto_encoder.AutoEncoder(**self._hyperparameter)