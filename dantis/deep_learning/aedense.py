...
class AEDense(algorithmbase.AlgorithmBase):
    """
    AEDense(hyperparameter: dict)
    Dense (Fully Connected) Autoencoder for anomaly detection in tabular or time series data.
    This class implements a fully connected (dense) autoencoder architecture for anomaly detection. The model is composed of several Dense layers forming an encoder and decoder, with a bottleneck (latent) layer for dimensionality reduction. It inherits from `algorithmbase.AlgorithmBase` and provides methods for model training, prediction, and anomaly scoring.
    Parameters
    ----------
    hyperparameter : dict
    ...
    def __init__(self, hyperparameter: dict):
        """
        Initialize the AEDense model with the given hyperparameters.
    ...
    def fit(self, x_train: np.array, y_train: np.array = None):
        """
        Trains the AEDense model with the training data.
    ...
    def _create_model(self):
        ...
        layer_list = [
            ...
            layers.Layer(layers.LayerType.DENSE, {"units": 128, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": 64, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": latent_dim, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": 64, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": 128, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": flat_dim, "activation": "linear"}).layer,
