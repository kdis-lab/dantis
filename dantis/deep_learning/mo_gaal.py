...
class MO_GAAL(algorithmbase.AlgorithmBase):
    """
    MO_GAAL(hyperparameter: dict)
    Multiple-Objective Generative Adversarial Active Learning (MO_GAAL) for unsupervised anomaly detection.
    This class implements the MO_GAAL algorithm, which uses multiple generators in a generative adversarial framework to detect anomalies in data. 
    It inherits from `algorithmbase.AlgorithmBase` and provides methods for model training, prediction, and anomaly scoring.
    The MO_GAAL model is based on adversarial learning, where multiple generators and a discriminator are trained to distinguish between normal and anomalous samples.
    Parameters
    ----------
    hyperparameter : dict
    ...
    ----------
    _model : object
        The underlying MO_GAAL model instance.
    _hyperparameter : dict
        Dictionary of hyperparameters for the model.
    Methods
    -------
    decision_function(x)
        Computes the anomaly score for the input data `x` using the trained MO_GAAL model.
    fit(x_train: np.ndarray, y_train: np.ndarray = None)
        Trains the MO_GAAL model on the provided training data.
    predict(x: np.ndarray = None) -> np.ndarray
        Returns the anomaly score for the input data `x`.
    save_model(path_model)
        Saves the trained MO_GAAL model to the specified path.
    load_model(path_model)
        Loads a MO_GAAL model from the specified path.
    set_hyperparameter(hyperparameter)
        Sets the hyperparameters for the MO_GAAL model.
    _create_model()
        Internal method to build the MO_GAAL model based on the current hyperparameters.
    Notes
    -----
    ...
    def __init__(self, hyperparameter: dict):
        """
        Initialize the MO_GAAL model with the given hyperparameters.
    ...
    def fit(self, x_train: np.array, y_train: np.array = None) -> np.array:
        ...
        """
        logging.info("Fitting MO_GAAL model...")
        logging.info("Don't use y_train in MO_GAAL model fitting, it is not used.")
    ...
    def _create_model(self):
        """
        Creates the MO_GAAL model with the provided hyperparameters.
        """
