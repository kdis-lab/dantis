...
class AECNN1D(algorithmbase.AlgorithmBase):
    """
    AECNN1D(hyperparameter: dict)

    Convolutional Autoencoder for time series data using 1D convolutions.

    This class implements a 1D Convolutional Autoencoder architecture for anomaly detection in time series data.
    The model consists of the following layers: Conv1D → MaxPooling1D → Conv1D → Flatten → Dense (latent space) → Dense → Reshape.
    ...
    def __init__(self, hyperparameter: dict):
        """
        Initialize the AECNN1D model with the given hyperparameters.
    ...
    def fit(self, x_train: np.array, y_train: np.array = None):
        """
        Trains the AECNN1D model with the training data.
    ...
    def _create_model(self):
        ...
        latent_dim = self._hyperparameter.get("latent_dim", 32)
        optimizer = self._hyperparameter.get("optimizer", "adam")
        loss = self._hyperparameter.get("loss", "mse")
        seq_len, n_feat = input_shape

        layer_list = [
            layers.Layer(layers.LayerType.CONV1D, {"filters": 32, "kernel_size": 5, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.MAX_POOLING1D, {"pool_size": 2}).layer,
            layers.Layer(layers.LayerType.CONV1D, {"filters": 16, "kernel_size": 3, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.FLATTEN, {}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": latent_dim, "activation": "relu"}).layer,
            layers.Layer(layers.LayerType.DENSE, {"units": seq_len * n_feat, "activation": "relu"}).layer,
