from pyod.models import rgraph # rgraph.RGraph
import numpy as np
import logging
...
class RGraph(algorithmbase.AlgorithmBase):
    """
    RGraph anomaly detection model wrapper.

    Parameters
    ----------
    hyperparameter : dict
        Dictionary containing hyperparameters for the RGraph algorithm.

    Methods
    -------
    fit(x_train, y_train=None)
        Fit the RGraph model using training data.
    decision_function(x)
    ...
    _create_model()
        Instantiate the RGraph model.
    """

    def __init__(self, hyperparameter: dict):
        """
        Initialize the RGraph detector with given hyperparameters.

        Parameters
        ----------
        hyperparameter : dict
            Dictionary of hyperparameters for the RGraph model.
        """
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray = None) -> np.ndarray:
        ...
        """
        logging.info("Fitting RGraph model...")
        logging.info("y_train is not used in RGraph fitting.")
    ...
    def save_model(self, path_model: str):
        """
        Save the RGraph model to the specified file.
    ...
    def load_model(self, path_model: str):
        """
        Load the RGraph model from the specified file.
    ...
    def _create_model(self):
        """
        Instantiate the RGraph model with the current hyperparameters.
        """
        self.set_hyperparameter(self._hyperparameter)
        self._model = rgraph.RGraph(**self._hyperparameter)