from pyod.models import sampling # sampling.Sampling
import numpy as np
import logging
...
class Sampling(algorithmbase.AlgorithmBase):
    """
    Sampling anomaly detection model wrapper.

    Parameters
    ----------
    hyperparameter : dict
        Dictionary containing hyperparameters for the Sampling algorithm.

    Methods
    -------
    fit(x_train, y_train=None)
        Fit the Sampling model using training data.
    decision_function(x)
    ...
    def __init__(self, hyperparameter: dict):
        """
        Initialize the Sampling detector with given hyperparameters.

        Parameters
        ----------
        hyperparameter : dict
            Dictionary of hyperparameters for the Sampling model.
        """
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray = None) -> np.ndarray:
        ...
        """
        logging.info("Fitting Sampling model...")
        logging.info("y_train is not used in Sampling fitting.")
    ...
    def save_model(self, path_model: str):
        """
        Save the Sampling model to the specified file.
    ...
    def load_model(self, path_model: str):
        """
        Load the Sampling model from the specified file.
    ...
    def _create_model(self):
        """
        Instantiate the Sampling model with the current hyperparameters.
        """
        self.set_hyperparameter(self._hyperparameter)
        self._model = sampling.Sampling(**self._hyperparameter)