from pyod.models import pca # pca.PCA
import numpy as np
import logging
...
class PCA(algorithmbase.AlgorithmBase):
    """
    PCA anomaly detection model wrapper.

    Parameters
    ----------
    hyperparameter : dict
        Dictionary containing the hyperparameters for the PCA algorithm.

    Methods
    -------
    fit(x_train, y_train=None)
        Fit the PCA model using training data.
    decision_function(x)
    ...
    def __init__(self, hyperparameter: dict):
        """
        Initialize the PCA detector with given hyperparameters.

        Parameters
        ----------
        hyperparameter : dict
            Dictionary of hyperparameters for the PCA model.
        """
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray = None) -> np.ndarray:
        ...
        logging.info("y_train is not used in PCA fitting.")
    ...
    def save_model(self, path_model: str):
        """
        Save the PCA model to the specified file.
    ...
    def _create_model(self):
        """
        Instantiate the PCA model with the current hyperparameters.
        """
        self.set_hyperparameter(self._hyperparameter)
        self._model = pca.PCA(**self._hyperparameter)