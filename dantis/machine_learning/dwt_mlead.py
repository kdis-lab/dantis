...
class DWTMLEAD(AlgorithmBase):
    """Adaptador para el algoritmo DWT_MLEAD.

    Comportamiento:
    - `fit` no hace nada (algoritmo no supervisado sin entrenamiento).
    - `decision_function` devuelve la puntuación por punto devuelta por
      `DWT_MLEAD.detect()`.
    - `predict` aplica un umbral sobre las puntuaciones para producir 0/1.

    Parámetros esperados en `hyperparameter`:
    - start_level: int
    - quantile_boundary_type: str (por defecto: "percentile")
    - quantile_epsilon: float
    - threshold: Optional[float] (umbral para `predict`; si None, usa >0)
    - track_coefs: bool
    """

    def __init__(self, hyperparameter: dict | None = None) -> None:
        super().__init__(hyperparameter or {})
        self._detector: Optional[DWT_MLEAD] = None

    def fit(self, x_train: np.ndarray, y_train: np.ndarray = None):
        # No training requerido; almacenar datos si se desea
        return self

    def decision_function(self, x: np.ndarray) -> np.ndarray:
        # Instantiate detector with hyperparameters from self._hyperparameter
        hp = self._hyperparameter or {}
        start_level = int(hp.get("start_level", 3))
        quantile_boundary_type = hp.get("quantile_boundary_type", "percentile")
        quantile_epsilon = float(hp.get("quantile_epsilon", 0.01))
        track_coefs = bool(hp.get("track_coefs", False))

        # Ensure 1-D array
        arr = np.asarray(x).ravel()
        self._detector = DWT_MLEAD(arr,
                                   start_level=start_level,
                                   quantile_boundary_type=quantile_boundary_type,
                                   quantile_epsilon=quantile_epsilon,
                                   track_coefs=track_coefs)
        scores = self._detector.detect()
        # Normalize scores to be non-negative floats
        scores = np.asarray(scores, dtype=float)
        # If all-zero, return as is
    ...
    def predict(self, x: np.ndarray) -> np.ndarray:
        ...
        if threshold is None:
            # default: any positive score is anomaly
